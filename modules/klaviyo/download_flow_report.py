...
class FLOW:
    ...
    def download_last_month_report(self) : 
        # Click on "Flow-Sagar" hyperlink
        hyperlink_xpath = '#AppShell-SiteBody > div > div > div > div > div > div > div > div > div > div > div.DataTable-sc-7x0ld0-0.inelmj > table > tbody > tr:nth-child(4) > td.Cell__StyledCell-sc-1gctzeb-0.iedUOx > div > div > a > span > span > div'
        ...
        new_file_path_last_month = os.path.join(self.local_file_path, f"flow_{last_month}.csv")
    ...
    def download_current_month_report(self) :
        ...
        new_file_path_curr_month = os.path.join(self.local_file_path, f"flow_{formatted_date}.csv")
